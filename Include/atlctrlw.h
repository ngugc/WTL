...
// Implementation
	...
	bool DisplayChevronMenu()
	{
		...
		for(int i = 0; i < nCount; i++)
		{
			REBARBANDINFO rbbi = { static_cast<UINT>(RunTimeHelper::SizeOf_REBARBANDINFO()), RBBIM_CHILD | RBBIM_STYLE };
	...
	{
		// refresh our font
		NONCLIENTMETRICS info = { static_cast<UINT>(RunTimeHelper::SizeOf_NONCLIENTMETRICS()) };
...
// Overrideables
	...
	void UpdateRebarBandIdealSize()
	{
		...
		for(int i = 0; i < nCount; i++)
		{
			REBARBANDINFO rbi = { static_cast<UINT>(RunTimeHelper::SizeOf_REBARBANDINFO()), RBBIM_CHILD | RBBIM_CHILDSIZE | RBBIM_IDEALSIZE };
...
// need conditional code because types don't match in winuser.h
		...
		if(bMaxOld != m_bChildMaximized)
		{
			...
			{
#if (_WIN32_IE >= 0x0500)
				REBARBANDINFO rbi = { static_cast<UINT>(RunTimeHelper::SizeOf_REBARBANDINFO()), RBBIM_CHILD | RBBIM_CHILDSIZE | RBBIM_IDEALSIZE | RBBIM_STYLE };
				::SendMessage(GetParent(), RB_GETBANDINFO, i, (LPARAM)&rbi);
				if(rbi.hwndChild == m_hWnd)
				{
